...
def fix():
    ...
    # Add <style> for the text button hover effect right before </head> if not exists
    style_block = """
    <style>
        .dl-btn-text {
            display: flex;
...
"""
    if '.dl-btn-text' not in html:
        html = html.replace('</head>', style_block + '</head>')

    # 1. Update Typography of the Header and Header Alignment
    # The header is inside <div class="dl-header" ...>
    # We will replace the whole header container
    old_header_regex = r'<div class="dl-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: \d+px;">([\s\S]*?)</div>\s*(?:<!--)?\s*<div class="dl-grid"'
    
    new_header = """<div class="dl-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 40px;">
                <h1 class="dl-title" style="font-size: 56px; font-weight: 500; line-height: 1.1; letter-spacing: -0.02em; margin: 0; color: #1a1a1a;">Download Nagi<br>for Web</h1>
                <a href="#" class="dl-changelog" style="display: inline-block; padding: 8px 16px; background-color: #f1f3f4; color: #1a1a1a; text-decoration: none; border-radius: 20px; font-size: 14px; font-weight: 500; margin-top: 14px;">View changelog</a>
            </div>
            
            <div class="dl-grid" """
    html = re.sub(old_header_regex, new_header, html)

    # 2. Update Grid Gap to 20px
    html = re.sub(r'<div class="dl-grid" style="([^"]*)gap:\s*\d+px;([^"]*)">', r'<div class="dl-grid" style="\1gap: 20px;\2">', html)

    # 3. Define new Cards
    # Using flex-direction: column to make them equal height and push requirements to the bottom
    web_card = """<div class="dl-card" style="padding: 28px 32px; flex: 1; display: flex; flex-direction: column;">
                    ...
                    <!-- Invisible placeholder for secondary button to match height -->
                    <div style="padding: 16px 24px; font-size: 14px; margin-bottom: 40px; visibility: hidden; line-height: 1; box-sizing: border-box;">Notify me</div>
                    
                    <div class="dl-requirements" style="margin-top: auto;">
                        <hr style="border: 0; border-top: 1px solid #eaeaea; margin: 0 0 20px 0; width: 100%;">
                        <h4 style="font-size: 11px; color: #1a1a1a; font-weight: 600; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.08em;">Works on any modern browser</h4>
                        <p style="font-size: 12px; color: #5f6368; line-height: 1.6; margin: 0;">Chrome 90+, Firefox 88+, Safari 14+, Edge 90+</p>
                    </div>
                </div>"""

    macos_card = """<div class="dl-card" style="padding: 28px 32px; flex: 1; display: flex; flex-direction: column;">
                    <h3 class="dl-os-title" style="font-size: 18px; font-weight: 500; margin-bottom: 30px; display: flex; align-items: center; gap: 12px; color: #1a1a1a;">
                    ...
                    <a href="javascript:void(0);" class="dl-btn-primary" style="display: flex; justify-content: center; align-items: center; width: 100%; padding: 16px 24px; border-radius: 40px; background-color: #F0F0F0; color: #5f6368; text-decoration: none; font-weight: 500; font-size: 14px; margin-bottom: 12px; box-sizing: border-box; line-height: 1;">Coming soon</a>
                    <a href="javascript:void(0);" class="dl-btn-text">Notify me</a>
                    
                    <div class="dl-requirements" style="margin-top: auto;">
                        <hr style="border: 0; border-top: 1px solid #eaeaea; margin: 0 0 20px 0; width: 100%;">
                        <h4 style="font-size: 11px; color: #1a1a1a; font-weight: 600; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.08em;">Minimum Requirements</h4>
                        <p style="font-size: 12px; color: #5f6368; line-height: 1.6; margin: 0;">macOS 12 (Monterey) or later. Apple Silicon + Intel supported.</p>
                    </div>
                </div>"""

    windows_card = """<div class="dl-card" style="padding: 28px 32px; flex: 1; display: flex; flex-direction: column;">
                    <h3 class="dl-os-title" style="font-size: 18px; font-weight: 500; margin-bottom: 30px; display: flex; align-items: center; gap: 12px; color: #1a1a1a;">
    ...
    print("Precision typography and layout updates applied!")
